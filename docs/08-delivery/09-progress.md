...
| Phase | **Checkpoint 7 — interface and hardening** (active, partially delivered — see the Checkpoint 7 section for exactly what exists); **Checkpoint 6 merged** via upstream PR #9 at `6195700` with post-merge CI green; Checkpoint 5 merged via upstream PR #8 at `c8cfb59`; Checkpoints 0–4 in PRs #1–#4 |
...
| Remaining Checkpoint 7 scope | **not delivered**: the complete resource families of the API plan (spec/assumptions, concepts, bible, plans, production mutations, canon correction/rollback/dependency promotion, costs/budgets, export lifecycle), SSE `GET /v1/jobs/{id}/events`, job pause/resume/cancel execution semantics, the Temporal decision and `apps/worker`, `apps/web` and all 11 UI screens, correction/regeneration/retcon operator flows, TXT/DOCX export artifacts, observability/metrics, rate limiting, and the deployment/operator runbooks. None of this is claimed as done |

Honest scope note (ADR-0043): Checkpoint 7 is the largest checkpoint in the roadmap. What exists today is
its platform and API foundation, delivered to the same evidence bar as earlier checkpoints — real
PostgreSQL tests, no suppressions, no placeholder routes. The table above is the authoritative statement of
what does and does not exist; the roadmap's Checkpoint 7 description remains the target, not a claim.
